...
type App struct {
	...
	StartUpActive	  	string 	`toml:"startup_active" comment:"Which panel should be active on startup: 'search', 'side', 'main', 'player'"`
	StartUpPlayerView 	bool 	`toml:"startup_player_view"`
	PlayOnStartUp		bool 	`toml:"play_on_startup"`
	StartUpLoopMode		string	`toml:"startup_loop_mode" comment:"'no' 'all' 'one' 'keep'"`
}
